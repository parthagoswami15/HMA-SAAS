...
export class PatientPortalController {
  ...
  updateProfile(@Body() updateDto: any, @Req() req: any) {
    return this.service.updateProfile(req.user.id, updateDto, req.user.tenantId);
  }

  @Get('my-appointments')
  getMyAppointments(@Req() req: any, @Query() query: any) {
    return this.service.getMyAppointments(req.user.id, req.user.tenantId, query);
  }

  @Post('book-appointment')
  bookAppointment(@Body() createDto: any, @Req() req: any) {
    return this.service.bookAppointment(req.user.id, createDto, req.user.tenantId);
  }
