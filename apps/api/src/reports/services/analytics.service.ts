...
export class AnalyticsService {
  ...
  async getDailyCollections(tenantId: string, filters: FinancialReportDto): Promise<any> {
  ...
  async getRevenueAnalysis(tenantId: string, filters: RevenueAnalysisDto): Promise<any> {
  ...
  async getGSTReport(tenantId: string, filters: GSTReportDto): Promise<any> {
  ...
  async getAgingAnalysis(tenantId: string, filters: AgingAnalysisDto): Promise<any> {
  ...
  async getCaseMixAnalysis(tenantId: string, filters: CaseMixDto): Promise<any> {
  ...
  async getInfectionRates(tenantId: string, filters: InfectionRateDto): Promise<any> {
    const { startDate, endDate, infectionTypes, departments, includeHAI, includeCAI } = filters;

    // This would typically come from a separate infection tracking table
  ...
  async getReadmissionsAnalysis(tenantId: string, filters: ReadmissionDto): Promise<any> {
    ...
    // This is a complex query that would need to identify readmissions
  ...
  async getTurnaroundTimes(tenantId: string, filters: TATDto): Promise<any> {
  ...
  async getOccupancyReport(tenantId: string, filters: OccupancyDto): Promise<any> {
    const { startDate, endDate, departments, bedTypes, granularity, includeForecasting } = filters;

    // This would typically come from bed management and admission data
    const mockData = [
      {
        date: '2024-01-15',
        department: 'General Medicine',
  ...
  async getLengthOfStayAnalysis(tenantId: string, filters: LOSDto): Promise<any> {
    const { startDate, endDate, departments, diagnosisCodes, admissionTypes, includeALOS, includeVariances } = filters;

    // Mock data for length of stay analysis
    const mockData = [
      {
        department: 'Cardiology',
        diagnosis: 'MI',
  ...
  async getBedTurnoverAnalysis(tenantId: string, filters: BedTurnoverDto): Promise<any> {
    const { startDate, endDate, departments, bedTypes, includeTurnoverRate, includeUtilizationRate } = filters;

    // Mock data for bed turnover
    const mockData = [
      {
        department: 'ICU',
        bed_type: 'ICU',
  ...
  async getPharmacyExpiryReport(tenantId: string, filters: PharmacyExpiryDto): Promise<any> {
    const { expiryBefore, drugCategories, manufacturers, alertDays, includeExpired, includeValue } = filters;

    // Mock data for pharmacy expiry
    const mockData = [
      {
  ...
  async getPatientAcquisition(tenantId: string, filters: PatientAcquisitionDto): Promise<any> {
    const { startDate, endDate, acquisitionType, channels, includeConversionRates, includeDemographics } = filters;

    // Mock data for patient acquisition
    const mockData = [
      {
  ...
  async getPatientRetention(tenantId: string, filters: PatientRetentionDto): Promise<any> {
    const { startDate, endDate, retentionPeriod, segments, includeCohortAnalysis, includeChurnRate } = filters;

    // Mock data for patient retention
    const mockData = [
      {
  ...
  async getReferralSourcesAnalysis(tenantId: string, filters: ReferralAnalysisDto): Promise<any> {
    const { startDate, endDate, sourceTypes, referringEntities, includeConversionRates, includeRevenueImpact } = filters;

    // Mock data for referral analysis
    const mockData = [
      {
  ...
  async getAnalyticsSummary(tenantId: string, options: { dateRange?: string; metrics?: string[] }): Promise<any> {
  ...
  private async getRevenueSummary(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
  ...
  private async getCollectionsSummary(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
    const result = await this.factBillingRepo
      .createQueryBuilder('billing')
      .leftJoin('billing.date', 'date')
      .where('billing.tenantId = :tenantId', { tenantId })
  ...
  private async getPatientVisitsSummary(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
  ...
  private async getLabTestsSummary(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
  ...
  private async getOccupancySummary(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
    // Mock data for occupancy summary
    return {
  ...
  async getTrendAnalysis(tenantId: string, metric: string, period: string, groupBy: string): Promise<any> {
  ...
  async getKPIDashboard(tenantId: string, category?: string): Promise<any> {
    const kpis: { [key: string]: any[] } = {
      financial: [
