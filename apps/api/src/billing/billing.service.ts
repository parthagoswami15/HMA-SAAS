...
export class BillingService {
  ...
  async createInvoice(dto: CreateInvoiceDto, tenantId: string) {
    try {
      ...
      // Calculate totals
      const totals = this.calculateInvoiceTotals(dto.items, dto.discountAmount || 0);
      ...
      const invoice = await this.prisma.$transaction(async (prisma) => {
        ...
        const itemsData = dto.items.map((item) => {
          const itemTotal = (item.quantity * item.unitPrice) - (item.discount || 0) +
            ((item.quantity * item.unitPrice - (item.discount || 0)) * (item.taxRate || 0) / 100);
      ...
      this.logger.log(`Invoice created: ${invoiceNumber} for tenant ${tenantId}`);
      return invoice;
    } catch (error) {
  ...
  async getInvoices(filters: InvoiceFilterDto, tenantId: string) {
    const { page = 1, limit = 10, patientId, status, startDate, endDate, search } = filters;
  ...
  async createPayment(dto: CreatePaymentDto, tenantId: string) {
    try {
      ...
      this.logger.log(`Payment created: ${paymentNumber} for invoice ${invoice.invoiceNumber}`);
      return payment;
    } catch (error) {
  ...
  async getPayments(filters: PaymentFilterDto, tenantId: string) {
    const { page = 1, limit = 10, invoiceId, paymentMethod, status, startDate, endDate } = filters;
    const skip = (page - 1) * limit;
