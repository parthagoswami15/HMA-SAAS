...
export class ResearchService {
  ...
  async create(createDto: any, tenantId: string) {
    const project = { id: Date.now().toString(), ...createDto, tenantId, status: 'ACTIVE', createdAt: new Date() };
    this.projects.push(project);
    return { success: true, message: 'Research project created (in-memory)', data: project };
  }
  ...
  async update(id: string, updateDto: any, tenantId: string) {
    ...
    this.projects[index] = { ...this.projects[index], ...updateDto, updatedAt: new Date() };
    return { success: true, message: 'Project updated', data: this.projects[index] };
  }

  async getStats(tenantId: string) {
    const total = this.projects.filter(p => p.tenantId === tenantId).length;
    const active = this.projects.filter(p => p.tenantId === tenantId && p.status === 'ACTIVE').length;
