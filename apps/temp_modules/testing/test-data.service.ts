...
export class TestDataService {
  ...
  async createTestData(tenantId: string) {
    try {
      // Create test patients using the PatientsService
      const testPatients: CreatePatientDto[] = [
        {
          firstName: 'John',
          lastName: 'Doe',
          dob: '1985-05-15',
      ...
      // Create patients in the database using the PatientsService
  ...
  async clearTestData(tenantId: string) {
    try {
      // Delete in reverse order of dependencies to respect foreign key constraints
      await this.prisma.prescription.deleteMany({ where: { tenantId } });
