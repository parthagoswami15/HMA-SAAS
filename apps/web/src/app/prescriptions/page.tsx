...
const PrescriptionsPage = () => {
  const [currentTab, setCurrentTab] = useState<'prescriptions' | 'pharmacy' | 'inventory' | 'reports'>('prescriptions');
  ...
  const [prescriptions] = useState<Prescription[]>([
    ...
    },
    {
      ...
      totalCost: 77.50,
      insuranceCoverage: 62.00,
      patientPayment: 15.50
    },
    {
      id: '3',
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'PARTIALLY_DISPENSED': return '#3b82f6';
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
            <div>
              <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.875rem', color: '#6b7280' }}>
                <strong>Prescribed:</strong> {new Date(prescription.prescriptionDate).toLocaleDateString()}
              </p>
              <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.875rem', color: '#6b7280' }}>
                <strong>Valid Until:</strong> {new Date(prescription.validUntil).toLocaleDateString()}
              </p>
              {prescription.dispensedDate && (
                <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.875rem', color: '#6b7280' }}>
                  <strong>Dispensed:</strong> {new Date(prescription.dispensedDate).toLocaleDateString()}
                </p>
              )}
            </div>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                <div>
                  ...
                  <strong>Age:</strong> {selectedPrescription.patientAge}<br />
                  <strong>Date:</strong> {new Date(selectedPrescription.prescriptionDate).toLocaleDateString()}
                </div>
                <div>
                  <strong>Doctor:</strong> {selectedPrescription.doctorName}<br />
                  <strong>Department:</strong> {selectedPrescription.department}<br />
                  <strong>Valid Until:</strong> {new Date(selectedPrescription.validUntil).toLocaleDateString()}
                </div>
                <div>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{ fontWeight: '600' }}>${selectedPrescription.totalCost.toFixed(2)}</span>
                </div>
                {selectedPrescription.insuranceCoverage && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', color: '#10b981' }}>
                    ...
                    <span style={{ fontWeight: '600' }}>-${selectedPrescription.insuranceCoverage.toFixed(2)}</span>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.125rem', fontWeight: '600' }}>
                  ...
                  <span style={{ color: '#1f2937' }}>${selectedPrescription.patientPayment.toFixed(2)}</span>
                </div>
              </div>
            </div>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid #e5e7eb' }}>
            {[
              ...
              { key: 'pharmacy', label: '🏪 Pharmacy', count: prescriptions.filter(p => p.status === 'ACTIVE').length },
            ...
            <div>
              <div style={{ 
                ...
                <h2 style={{ fontSize: '1.25rem', fontWeight: '600', color: '#1f2937' }}>
                  {currentTab === 'pharmacy' ? 'Pharmacy Queue' : 'Prescriptions'} ({filteredPrescriptions.length})
                </h2>
              </div>
        ...
        {currentTab === 'inventory' && (
          <Card>
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <p style={{ color: '#6b7280', marginBottom: '1rem' }}>
                Track medication stock levels, expiry dates, and reorder points. Inventory management system will be implemented here.
              </p>
              <Button variant="primary">
                Manage Inventory
              </Button>
        ...
        {currentTab === 'reports' && (
          <Card>
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <p style={{ color: '#6b7280', marginBottom: '1rem' }}>
                Comprehensive pharmacy analytics, medication usage patterns, and financial reporting will be available here.
              </p>
              <Button variant="primary">
