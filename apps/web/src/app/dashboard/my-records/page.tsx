...
export default function MyHealthRecordsPage() {
  ...
  return (
    <Stack gap="xl">
      <div>
        <Title order={2} mb="xs">My Health Records</Title>
      ...
      <Group>
        ...
        <Card shadow="sm" padding="lg" radius="md" withBorder style={{ flex: 1 }}>
          <Group justify="apart">
            <div>
              <Text size="sm" c="dimmed">Lab Tests</Text>
      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder>
        <Title order={4} mb="md">Medical History Timeline</Title>
        <Timeline active={-1} bulletSize={24} lineWidth={2}>
          {medicalRecords.map((record, index) => (
            <Timeline.Item 
              key={index}
              bullet={
                record.type === 'Consultation' ? <IconStethoscope size={12} /> :
                record.type === 'Lab Test' ? <IconFlask size={12} /> :
                <IconPill size={12} />
              }
              title={
                <Group gap="xs">
              ...
              <Accordion variant="separated">
                <Accordion.Item value="details">
                  ...
                  <Accordion.Panel>
                    <Stack gap="xs">
                      {record.doctor && (
                        <Text size="sm"><strong>Doctor:</strong> {record.doctor}</Text>
                      )}
                      {record.diagnosis && (
                        <Text size="sm"><strong>Diagnosis:</strong> {record.diagnosis}</Text>
                      )}
                      {record.prescription && (
                        <Text size="sm"><strong>Prescription:</strong> {record.prescription}</Text>
                      )}
                      {record.test && (
                        <Text size="sm"><strong>Test:</strong> {record.test}</Text>
                      )}
                      {record.status && (
                        <Badge color="green" variant="light">{record.status}</Badge>
                      )}
                      {record.vaccine && (
                        <Text size="sm"><strong>Vaccine:</strong> {record.vaccine}</Text>
                      )}
                      {record.notes && (
                        <Text size="sm" c="dimmed" mt="xs">{record.notes}</Text>
                      )}
                    </Stack>
                  </Accordion.Panel>
                </Accordion.Item>
      ...
      <Card shadow="sm" padding="md" radius="md" withBorder bg="blue.0">
        <Text size="sm" c="dimmed">
          <strong>Note:</strong> For any questions about your medical records, please contact your healthcare provider directly.
        </Text>
      </Card>
    </Stack>
  );
