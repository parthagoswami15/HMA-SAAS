...
interface StaffMember {
  ...
  email: string;
  phone: string;
  role: 'SUPER_ADMIN' | 'ADMIN' | 'DOCTOR' | 'NURSE' | 'LAB_TECHNICIAN' | 'RADIOLOGIST' | 'PHARMACIST' | 'RECEPTIONIST' | 'ACCOUNTANT';
  department: string;
  designation: string;
...
const StaffPage = () => {
  const [currentTab, setCurrentTab] = useState<'directory' | 'schedule' | 'performance'>('directory');
  ...
  const departments = ['Cardiology', 'Emergency', 'Pediatrics', 'Laboratory', 'Pharmacy', 'Radiology', 'Orthopedics'];
  const roles = ['DOCTOR', 'NURSE', 'LAB_TECHNICIAN', 'PHARMACIST', 'RADIOLOGIST', 'RECEPTIONIST', 'ADMIN'];
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      ...
      case 'LAB_TECHNICIAN': return '#3b82f6';
      case 'PHARMACIST': return '#f59e0b';
      case 'RADIOLOGIST': return '#ef4444';
    ...
    <Card variant="elevated" style={{ marginBottom: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginLeft: '1rem' }}>
          <Button size="sm" variant="outline" onClick={() => window.location.href = `/staff/${member.id}`}>
            View Profile
          </Button>
          <Button size="sm" variant="primary" onClick={() => window.location.href = `/staff/${member.id}/edit`}>
            Edit
          </Button>
          <Button size="sm" variant="secondary" onClick={() => window.location.href = `/staff/${member.id}/schedule`}>
            Schedule
          </Button>
        </div>
      </div>
              ...
              <Card variant="bordered">
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#f59e0b' }}>
                    {Math.round(staff.reduce((acc, s) => acc + s.performanceRating, 0) / staff.length * 10) / 10}
                  </div>
        ...
        {currentTab === 'schedule' && (
          <Card>
            <div style={{ textAlign: 'center', padding: '3rem' }}>
              ...
              <p style={{ color: '#6b7280', marginBottom: '2rem', maxWidth: '500px', margin: '0 auto 2rem auto' }}>
                Manage staff schedules, shifts, and work assignments. Create weekly schedules, handle shift swaps, and track attendance.
              </p>
              <Button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '2rem' }}>
                {staff.filter(s => s.isActive).map(member => (
                  <div key={member.id} style={{ textAlign: 'center' }}>
                    ...
                    <Button size="sm" variant="outline" onClick={() => window.location.href = `/staff/${member.id}/performance`}>
                      View Details
                    </Button>
                  </div>
                ))}
