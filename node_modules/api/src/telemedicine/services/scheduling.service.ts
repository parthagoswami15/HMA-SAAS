...
export class SchedulingService {
  ...
  async getAvailability(query: any, user: any) {
    ...
    const slotDuration = doctor.telemedicineSettings?.slotDuration || 30; // minutes
  ...
  async findEmergencyDoctor(tenantId: string) {
    ...
    const emergencyDoctors = await this.prisma.user.findMany({
      ...
      orderBy: { lastEmergencyConsultation: 'asc' }, // Prioritize doctors with fewer recent emergencies
    });
